...
const FIELDS = ['brand_id','name','type','contact_email','status','notes'];
const pick = (b: any) => Object.fromEntries(Object.entries(b).filter(([k]) => FIELDS.includes(k)));
async function GET__impl(request: NextRequest) {
  const { session, error } = await requireSession(request);
  if (error) return error;
  const brandId = request.nextUrl.searchParams.get('brandId');
  try { return NextResponse.json(await partnersRepo.list(session.accountId, brandId)); }
...
export const GET = withApi(GET__impl as any, { route: "/api/partners", method: "GET" });
export const POST = withApi(POST__impl as any, { route: "/api/partners", method: "POST" });
