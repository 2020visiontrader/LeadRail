...
export async function schedulePostizPost(post: PostizPost) {
  ...
  if (!response.ok) {
    throw new Error(`Postiz error: ${response.statusText}`);
  }
...
export async function handlePostizWebhook(event: any) {
  ...
  if (eventType === 'published') {
    console.log(`Post ${postId} published to ${data.platforms.join(', ')}`);
  }
  if (eventType === 'analytics') {
    console.log(`Post ${postId} analytics:`, data.metrics);
  }
}